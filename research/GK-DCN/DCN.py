"""
Deformable Convolutional Network (DCN) Model for Geochemical anomaly identification
======================================================================

Main Modules:
- DeformableConv2d: Custom implementation of a Deformable Convolutional Layer 
  using 'torchvision.ops.DeformConv2d' with an offset and a modulation mask branch.
- Model Training: Standard PyTorch training loop using Adam optimizer and BCELoss.
  Save model weights ('model.pth') for subsequent visualization of offsets
- Evaluation: 'evaluate_model' function computes and prints comprehensive metrics 
...
print(f"Verify data dimensions: {verify_data.shape}")
...
with torch.no_grad():
    ...
    if all_patches:
        ...
        print(f"Full image prediction completed. {len(probability_value)} probability values generated.")
    else:
        print("Warning: No windows available for prediction.")
        exit()
...
dataframe.to_csv('dcn_output.csv', index=False)
print("\nPrediction probability map saved to 'dcn_output.csv'")
