"""
This project implements the GK-DCN model, which is a deep learning architecture specifically designed for geochemical anomaly detection. 
The model uses a deformable convolutional network (DCN) to process multichannel geochemical data and introduces a novel Geological Loss, integrating prior geological knowledge (geological potential maps) as soft constraints during training.
The core objective is to improve the accuracy and geological plausibility of geochemical anomaly detection by aligning the model's predicted outputs with auxiliary predictions based on geological potential.
"""
...
window_size = 9
all_channel = 39
model_save_path = 'GK_DCN_model.pth'
...
# =============================================================================
# Definition of Deformable Conv Layer
# =============================================================================
...
class DeformableConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1, stride=1):
        ...
        self.dcn = DeformConv2d(
            in_channels, out_channels, kernel_size=kernel_size, padding=padding, stride=stride, bias=True
        )
...
class GK_DCN(nn.Module):
    def __init__(self):
        super(GK_DCN, self).__init__()
        # DCN Feature Extraction Layers
    ...
    def forward(self, x, geo_potential_patch=None):
        ...
        x = self.pool2(self.relu(self.bn2(self.offsetconv1(x))))
        x = self.relu(self.bn3(self.offsetconv2(x)))
        ...
        if geo_potential_patch is not None:
            ...
            # geo_potential_patch shape: (B, 1) -> (B, 1, 1, 1)
...
class GeologicalAlignmentLoss(nn.Module):
    def __init__(self, geo_lambda=0.1):
        ...
        self.bce_loss = nn.BCELoss(reduction='mean') # Basic Loss L_CNN
        self.l2_loss = nn.MSELoss(reduction='mean') # Distance Loss L_geology
    ...
    def forward(self, p_model, targets, p_geology):
        ...
        if p_geology is not None:
            # L_geology = L2(P_model, P_geology)
...
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.title('Model Accuracy (GK-DCN Alignment)')
plt.legend()
plt.grid(True)
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Model Loss (GK-DCN Alignment)')
plt.legend()
plt.grid(True)
